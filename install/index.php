		...
		try {
			...
			if (strtolower($engine) == 'mariadb') {
				if ($version < 10.3) {
					...
					$response['message'] = 'MariaDB 10.3+ Or MySQL 5.7+ Required. <br> version is MariaDB ' . $version;
				}
			} else {
				if ($version < 5.7) {
					$response['error'] = 'error';
					$response['message'] = 'MariaDB 10.3+ Or MySQL 5.7+ Required. <br> version is MySQL ' . $version;
				}
			}
		} catch (Exception $e) {
			$response['error'] = 'error';
